...
function parseSheet(sheet, headIndex) {
  ...
  Editor.log('\t parsing sheet', sheet.name);
...
function parseHead(sheet, headIndex) {
  ...
  // Editor.log("\t\t parsing head", headIndex, headRow);
  ...
  row.forEach((cell, index) => {
    ...
    switch (type) {
      ...
      case DataType.DATE:
        ...
        break;
      case DataType.TRIM:
        result[name] = trim(cell.toString());
        break;
      ...
      case DataType.NUMBER:
        ...
        if (isNumber(cell)) {
          ...
          Editor.warn("type error at [" + rowIndex + "," + index + "]," + cell + " is not a number");
        }
        break;
      case DataType.BOOL:
        result[name] = toBoolean(cell);
      ...
      case DataType.OBJECT: //support {number boolean string date} property type
        ...
        break;
      case DataType.ARRAY_TRIM:
        result[name] = parseBasicArrayField(cell, arraySeparator, true);
        break;
      ...
      default:
        // foo#[]| 处理自定义数组分隔符
        if (type !== undefined && type.indexOf(DataType.ARRAY) !== -1) {
          ...
          Editor.error('unrecognized type', '[' + rowIndex + ',' + index + ']', cell, typeof(cell));
        }
        break;
    }
    // }
 ...
 * parse simple array.
 */
function parseBasicArrayField(array, arraySeparator, isTrim) {
  let basic_array;
  ...
  // Editor.log("basic_array", result + "|||" + cell.value);
  // field[key] = result;
  return result;
}
