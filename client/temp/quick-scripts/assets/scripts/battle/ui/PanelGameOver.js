(function() {"use strict";var __module = CC_EDITOR ? module : {exports:{}};var __filename = 'preview-scripts/assets/scripts/battle/ui/PanelGameOver.js';var __require = CC_EDITOR ? function (request) {return cc.require(request, require);} : function (request) {return cc.require(request, __filename);};function __define (exports, require, module) {"use strict";
cc._RF.push(module, '660aeovmnZDu641t/+QJ1YK', 'PanelGameOver', __filename);
// scripts/battle/ui/PanelGameOver.js
...
var RANK = ['1st', '2nd', '3rd', '4th', '5th', '6th', '7th', '8th'];
...
cc.Class({
    ...
    init: function init(world) {
        ...
        if (!this.first) {
            ...
            if (GameData.instance.isShowLog()) {
                console.log('基础金币', Tools.getItemOrFinalItem(rankData.getGold, rank - 1), '段位加成', rankData.goldMultiRate, '成长加成', growGoldParam);
            }
            ...
            if (this.multiplyGoldNode.active) {
                ...
                if (this.stage === 2) {
                    AdvertMgr.instance.loadAdver(AdverType.MultipGold, function (hasAdver) {
                        _this.multipShareIcon.active = !hasAdver;
                        _this.multipAdverIcon.active = hasAdver;
            ...
            if (getStar === 1) {
                ...
                if (!isLevelChange) {
                    ...
                    self.animAddStar.node.position = shineStar.position;
                    this.addStarCallback = function () {
                        self.animAddStar.once('finished', function () {
                            shineStar.active = true;
                            setTimeout(function () {
                ...
                if (!isLevelChange) {
                    ...
                    if (wasKing) {
                        ...
                        this.subRefreshCallback = function () {
                            ...
                            _this.kingStar.active = true;
                            self.animSubStar.once('finished', function () {
                ...
                setTimeout(function () {
                    if (protectWay === 0 || protectWay === 1) {
                        _this.showPanelProtect(rankData, newRankData, isLevelChange, protectWay);
                    } else {
                        if (_this.subRefreshCallback) {
                            _this.subRefreshCallback();
                        }
    ...
    refreshData: function refreshData(isProtect) {
        var self = this;
    ...
    showMultAgain: function showMultAgain(count, mult) {
        var _this2 = this;
        ...
        var func = function func(hasAdver) {
            _this2.multAgainAdverIcon.active = hasAdver;
            _this2.multAgainShareIcon.active = !hasAdver;
            if (hasAdver) {
    ...
    onMultAgainBtnClick: function onMultAgainBtnClick() {
        var _this3 = this;

        var self = this;
        var multipCallback = function multipCallback(success) {
            if (success) {
                var count = _this3.multAgainCount;
                ...
                _this3.world.uiMgr.showTips(Tools.getStringByFormat(ConfigData.instance.getUITipStr(17), count));
                _this3.world.restartGame();
            }
        };
    ...
    onCLoseMultAgain: function onCLoseMultAgain() {
        this.panelMultAgain.active = false;
        PlayerData.instance.resetDayMultAgainCDCount(ConfigData.instance.clientData.dayMultAgainCDCount);
    ...
    showMultAgainShare: function showMultAgainShare(multipCallback) {
        ShareMgr.share(ShareType.MultipAgain, multipCallback);
    },

    showMultAgainAdvert: function showMultAgainAdvert(multipCallback) {
        var self = this;
        // 打开广告失败时回调,失败回调
    ...
    onShareBtnClick: function onShareBtnClick() {
        var self = this;
        ShareMgr.share(ShareType.WIN, function (isSuccess) {
            if (isSuccess) {
    ...
    onMultiplyBtnClick: function onMultiplyBtnClick() {
        var self = this;
        var multipCallback = function multipCallback(success) {
            if (success) {
    ...
    showShare: function showShare(multipCallback) {
        ShareMgr.share(ShareType.MultipGold, multipCallback);
    },

    showAdvert: function showAdvert(multipCallback) {
        var self = this;
        // 打开广告失败时回调,失败回调
        var errFunc = function errFunc() {
    ...
    updateRankBar: function updateRankBar(dt) {
    ...
    updateLuckyBar: function updateLuckyBar(dt) {
    ...
    onLuckyRewardBtnClick: function onLuckyRewardBtnClick() {
        this.luckyRewardNode.active = true;
        this.onLuckyMultipBtnClick();
    },

    onLuckyMultipBtnClick: function onLuckyMultipBtnClick() {
        this.blockNode.active = true;
        ...
        var closeFunc = function closeFunc(success) {
            if (success) {
                ...
                xNodes[0].y = 0;
                for (var i = 1; i < 10; i++) {
                    var mult = Tools.getRandomInt(5, 10);
                    str += mult + '\n\r';
                    if (xNodes[i]) {
    ...
    showLuckyShare: function showLuckyShare(closeFunc) {
        ShareMgr.share(ShareType.MultipGold, closeFunc);
    },

    showLuckyAdver: function showLuckyAdver(closeFunc) {
        var self = this;
        // 打开广告失败时回调,失败回调
        var errFunc = function errFunc() {
    ...
    onCloseLuckyNodeBtnClick: function onCloseLuckyNodeBtnClick() {
        this.luckyRewardNode.active = false;
    },

    onProtectBtnClick: function onProtectBtnClick() {
        var self = this;
        // 关闭回调
        var closeFunc = function closeFunc(success) {
            if (success) {
    ...
    showProtectShare: function showProtectShare(closeFunc) {
        ShareMgr.share(ShareType.ProtectStar, closeFunc);
    },

    showProtectAdver: function showProtectAdver(closeFunc) {
        var self = this;
        var errFunc = function errFunc() {
            self.showProtectShare(closeFunc);
        };
    ...
    onReduceProtectBtnClick: function onReduceProtectBtnClick() {
    ...
    showPanelProtect: function showPanelProtect(curData, nextData, isLevelDown, protectWay) {
    ...
    refreshProtectRankStar: function refreshProtectRankStar(starNode, starCount) {
    ...
    refreshMultipGoldBtn: function refreshMultipGoldBtn() {
